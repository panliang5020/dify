...
KingbaseES 的 Python 驱动 `ksycopg2` 由人大金仓官方提供，**不在 PyPI 公共仓库**，需从官方渠道获取后手动安装：
...
# 方式二：若官方提供了私有 PyPI 源
pip install ksycopg2 --index-url https://your-kingbase-pypi-mirror/simple/
```

> **说明**：`ksycopg2` 是人大金仓官方 Python 驱动，与标准 `psycopg2` 相互独立，安装时无需卸载已有的 `psycopg2-binary`，两者可共存。
...
- `DB_TYPE=kingbase`   → 方案头为 `kingbase8+ksycopg2`（使用官方 `ksycopg2` Python 驱动）
